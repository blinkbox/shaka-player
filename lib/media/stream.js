...
/**
 * The amount of content we will try to buffer by default when trying to switch bitrates.
 *
 * @type {number}
 */
shaka.media.Stream.switchBufferSizeSeconds = 3.0;
...
shaka.media.Stream.prototype.switch = function(streamInfo, immediate) {
  ...
  Promise.all(async).then(shaka.util.TypedBind(this,
      function() {
        // take the minimum buffer size for the stream switching operation else there will not be enough
        // data to switch smoothly.
        var switchBufferSize = Math.min(
            shaka.media.Stream.switchBufferSizeSeconds, shaka.media.Stream.bufferSizeSeconds);
        ...
        if (immediate) {
          ...
          var estimatedFetchBits =
              switchBufferSize * requiredBitsPerSecond;
