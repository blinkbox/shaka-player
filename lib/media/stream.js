...
/**
 * The amount of content we will initially try to buffer by default, after startup.
 *
 * @type {number}
 */
shaka.media.Stream.initialBufferSizeSeconds = 0;
  ...
  Promise.all(async).then(shaka.util.TypedBind(this,
      function() {
        ...
        var segmentRange = streamInfo.segmentIndex.getRangeForInterval(
            this.video_.currentTime, shaka.media.Stream.initialBufferSizeSeconds);
        if (!segmentRange) {
...
/**
 * Returns the shaka.media.Stream.bufferSizeSeconds of an instance.
 *
 * @return {number}
 * @private
 */
