...
shaka.media.Stream =
  ...
  /** @private {number} */
  this.initialBufferTime_ = Number(valueOrDefault(getParameterValue("shakaBufferInitial"), 5.0));

      /** @private {number} */
  this.bufferingGoal_ = Number(valueOrDefault(getParameterValue("shakaBufferGoal"), 120.0));

  /** @private {number} */
  this.switchBufferSize_ = Number(valueOrDefault(getParameterValue("shakaBufferSwitch"), 5.0));
...
shaka.media.Stream.prototype.start = function(streamInfo, initialBufferSize) {
  ...
  // force the initial buffer size to a specific value.
  initialBufferSize = this.initialBufferTime_;
  ...
  Promise.all(async).then(shaka.util.TypedBind(this,
      function() {
        ...
        if (immediate) {
          ...
          var estimatedFetchBits = this.switchBufferSize_ * requiredBitsPerSecond;
        ...
        var segmentRange = streamInfo.segmentIndex.getRangeForInterval(
            switchTime, this.switchBufferSize_);
        if (!segmentRange) {
          return Promise.reject(new Error('No segments available.'));
        }
