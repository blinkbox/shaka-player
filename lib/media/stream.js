 ...
 * @param {string} name of the querystring parameter.
 * @returns {string}
 */
...
/**
 * Return the value if available, else return the alternative value.
 * @param {string} value
 * @param alt alternative number.
 * @returns {*}
 */
function valueOrDefault(value, alt) {
  if (value) {
 ...
 * @const
 */
shaka.media.Stream.BUFFER_SIZE_SECONDS_  =
    Number(valueOrDefault(getParameterValue("shakaBufferGoal"), 120.0));
 ...
 * @const
 */
shaka.media.Stream.INITIAL_BUFFER_SIZE_SECONDS_ =
    Number(valueOrDefault(getParameterValue("shakaBufferInitial"), 5.0));
 ...
 * @const
 */
shaka.media.Stream.SWITCH_BUFFER_SIZE_SECONDS_ =
    Number(valueOrDefault(getParameterValue("shakaBufferSwitch"), 5.0));
...
/**
 * The amount of time to retain in the buffer when upgrading bitrates
 *
 * @private
 * @const {number}
 */
shaka.media.Stream.BUFFER_RETENTION_ON_BITRATE_UPGRADE_SECONDS = 5.0;
...
shaka.media.Stream.prototype.start = function(streamInfo) {
  ...
  Promise.all(async).then(shaka.util.TypedBind(this,
      ...
      function(results) {
        ...
        var initialBufferTime =shaka.media.Stream.INITIAL_BUFFER_SIZE_SECONDS_;
...
shaka.media.Stream.prototype.switch = function(streamInfo, immediate) {
  ...
  // Buffer retention:
  // Determine how much of the current buffer we want to retain before replacing (if at all). E.g. when
  // downgrading bitrates, we want to keep all the buffer (may as well play higher quality segments), but
  // when upgrading, retain just enough to avoid rebuffering during the switch (want to play the higher quality
  // segments as soon as possible, without causing rebuffering.
  var bufferTimeToRetain = 0;
  ...
  Promise.all(async).then(shaka.util.TypedBind(this,
      ...
      function(results) {
        ...
        // Calculate bufferTimeToRetain depending on whether we are upgrading or downgrading bitrate.
        var currentTime = this.getCurrentTime_();
        if (this.streamInfo_.bandwidth > streamInfo.bandwidth) {
          var referenceIndex = this.findNextNeededIndex_(currentTime);
          var reference = this.segmentIndex_.getReference(referenceIndex);
          bufferTimeToRetain = reference.startTime - currentTime;
        }
        else if (this.streamInfo_.bandwidth < streamInfo.bandwidth) {
          bufferTimeToRetain = Stream.BUFFER_RETENTION_ON_BITRATE_UPGRADE_SECONDS;
        }
        ...
        // Reset SBM differently depending if any buffer is being retained.
        if (bufferTimeToRetain == 0) {
          this.sbm_.reset();
        }
        else {
          // Reset all segments, except those being retained
          var currentTimeSegmentIndex = this.segmentIndex_.findReferenceIndex(currentTime);
          var retainedBufferSegmentIndex = this.segmentIndex_.findReferenceIndex(currentTime + bufferTimeToRetain);
          this.sbm_.resetWithRetention(currentTimeSegmentIndex, retainedBufferSegmentIndex);
        }
      ...
      function() {
        ...
        var segmentRange = this.segmentIndex_.getRangeForInterval(
            this.getCurrentTime_() + bufferTimeToRetain,
            bufferTime);
        if (!segmentRange) {
          return Promise.reject(new Error('No segments available.'));
        }
      ...
      function(error) {
        ...
        if (error.type != 'aborted') {
          if (error.type == 'net' && error.xhr.status == 0) {
            // Flag that this error will be retried. This allows clients who subscribe
            // to the error event can decide what action to take
