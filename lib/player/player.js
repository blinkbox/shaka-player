...
/**
 * @param {number} bufferSize The amount of content streams will try to buffer
 *    in seconds when switching.
 * @export
 */
shaka.player.Player.prototype.setSwitchStreamBufferSize = function(bufferSize) {
  shaka.asserts.assert(bufferSize >= 0);
  var buffer = (bufferSize < 0) ? 0 : bufferSize;
  shaka.media.Stream.switchBufferSizeSeconds = buffer;
};


/**
 * Returns the seconds of content streams will try to buffer when switching.
 * @return {number}
 * @export
 */
shaka.player.Player.prototype.getSwitchStreamBufferSize = function() {
  return shaka.media.Stream.switchBufferSizeSeconds;
};
