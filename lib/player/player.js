...
shaka.player.Player = function(video) {
  ...
  this.videoSourceConfig_ = {
    ...
    'rangeRequestTimeout': shaka.player.Defaults.RANGE_REQUEST_TIMEOUT
  };
...
shaka.player.Player.prototype.configure = function(config) {
  ...
  var rangeRequestTimeout =
      MapUtils.getNumber(config, 'rangeRequestTimeout', 0);
  if (rangeRequestTimeout != null) {
    this.videoSourceConfig_['rangeRequestTimeout'] = rangeRequestTimeout;
  }
  ...
  if (preferredLanguage != null) {
    this.lang_ = shaka.util.LanguageUtils.normalize(preferredLanguage);
  }

  var restrictions = MapUtils.getAsInstanceType(
      config, 'restrictions', shaka.player.DrmSchemeInfo.Restrictions);
...
shaka.player.Player.prototype.getConfiguration = function() {
  return {
    ...
    'streamBufferSize': this.videoSourceConfig_['streamBufferSize'],
    'licenseRequestTimeout': this.videoSourceConfig_['licenseRequestTimeout'],
    'mpdRequestTimeout': this.videoSourceConfig_['mpdRequestTimeout'],
    'rangeRequestTimeout': this.videoSourceConfig_['rangeRequestTimeout'],
