...
/**
 * @param {number} bufferSize The amount of content streams will try to buffer in seconds during initialization.
 * @export
 */
shaka.player.Player.prototype.setInitialStreamBufferSize = function(bufferSize) {
  shaka.asserts.assert(bufferSize >= 0);
  var buffer = (bufferSize < 0) ? 0 : bufferSize;
  shaka.media.Stream.initialBufferSizeSeconds = buffer;
};

/**
 * Returns the seconds of content streams will try to initially buffer before playback starts.
 * @export
 */
shaka.player.Player.prototype.getInitialStreamBufferSize = function() {
  return shaka.media.Stream.initialBufferSizeSeconds;
};
