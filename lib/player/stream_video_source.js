 ...
 *    some minimum amount of content, which is determined from the ManifestInfo.
 ...
 *
 * @param {!Object.<string, !shaka.media.StreamInfo>} streamInfosByType
 * @param {{start: number, end: number}} streamLimits The initial stream limits.
 * @private
 */
...
shaka.player.StreamVideoSource.prototype.onAllStreamsStarted_ = function(
  ...
  // Compute the timestamp correction.
...
/**
 * Corrects the MediaSource's duration and append windows, corrects the video's
 * current time, and then begins playback.
 *
...
shaka.player.StreamVideoSource.prototype.beginPlayback_ = function(
  ...
  // Determine the corrected current time and adjust the video's current time
  // as necessary. If |timestampCorrection| is 0 then don't modify
  // video.currentTime to avoid firing an unnecessary 'seeking' event.
  //
  // Note that its particularly important to adjust the video's current time if
  // |timestampCorrection| is positive; otherwise, some streams may get
 ...
 *
 * @param {{start: number, end: number}} streamLimits The current stream limits.
 * @private
 */
shaka.player.StreamVideoSource.prototype.setUpMediaSource_ = function(
  ...
  // and allow the user agent to seek.
  if (this.manifestInfo.live) {
    ...
    // after timestamp correction). If we insert a segment into the
    // SourceBuffer that ends after the MediaSource's duration then the
    // MediaSource's duration will increase, and if the MediaSource's duration
    // increases beyond the seekable range (by some non-negligible amount) then
    // the playhead will be able to move into an unseekable range during
    // playback, which will interfere with normal end-of-stream behavior and
    // buffering detection (see #155).
    //
