...
shaka.player.StreamVideoSource.prototype.startStreams_ = function(
  ...
  for (var type in this.streamsByType_) {
    ...
    var streamInfo = streamInfosByType[type];
    this.stats_.logStreamChange(streamInfo);
    stream.start(streamInfo, this.manifestInfo.minBufferTime);
  }
...
shaka.player.StreamVideoSource.prototype.onAllStreamsStarted_ = function() {
  ...
  shaka.log.info(
      'min/max timestamp corrections',
      [this.minTimestampCorrection_, this.maxTimestampCorrection_]);
  ...
  // Immediately correct the initial set of SegmentIndexes.
  // TODO: Correct each SegmentIndex with its specific timestamp correction.
  // Currently, we can do this for the initial active streams, but we cannot
  // do this for the streams we switch to.
  var async = shaka.util.MapUtils.values(this.streamsByType_)
      .map(function(stream) {
  ...
  Promise.all(async).then(shaka.util.TypedBind(this,
      /** @param {!Array.<!shaka.media.SegmentIndex>} segmentIndexes */
      function(segmentIndexes) {
        // Regardless of whether the timestamp correction is positive or
        // negative, correct by the max: if we corrected by the min then some
        // SegmentIndexes would end up behind their media timeline.
        var correct = (function(correction, index) {
          index.correct(correction);
        }).bind(null, this.maxTimestampCorrection_);

        segmentIndexes.forEach(correct);

        this.onAllStreamsStarted(segmentIndexes);
        this.setUpdateTimer_(0);

        // Create and correct all other SegmentIndexes "in the background".
        var async = this.streamSetsByType.getAll()
            .map(function(streamSetInfo) { return streamSetInfo.streamInfos; })
            .reduce(function(all, part) { return all.concat(part); }, [])
            .map(function(streamInfo) {
              // TODO: To mitigate potential gaps in the stream, disallow
              // stream switching until all SegmentIndexes have been created
              // and corrected.
              return streamInfo.segmentIndexSource.create().then(correct);
            });
 ...
 *
 * The default implementation adjusts the video's current time by the timestamp
 * correction and then starts the video.
 *
 * @param {!Array.<!shaka.media.SegmentIndex>} segmentIndexes
 * @protected
 */
