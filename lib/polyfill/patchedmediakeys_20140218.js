...
shaka.polyfill.PatchedMediaKeys.v20140218.MediaKeySystemAccess =
    function(keySystem, supportedConfigurations) {
        ...
        // As per the spec, this should be a DOMException, but there is not a public constructor for this
 ...
 *
 * @param {Uint8Array|null} initData
 * @private
 * return {!Uint8Array}
 */
shaka.polyfill.PatchedMediaKeys.v20140218.NormaliseInitData_ = function(initData) {
    if (!initData) {
        return initData;
    }
    ...
    for (var i = 0; i < pssh.dataBoundaries.length; i++) {
        ...
        // If we have not seen the key for this subarray before, add
        // to the deduped list
        var key = shaka.util.Uint8ArrayUtils.key(currPssh);
        if (!dedupedKeys[key]) {
            dedupedKeys[key] = true;
            dedupedInitData.push(currPssh);
            dedupedInitDataSize += currPssh.length;
        }
    }

    // Concat array of Uint8Arrays back into a single Uint8Array
    var normalisedInitData = new Uint8Array(dedupedInitDataSize);
    var offset = 0;
    for (var i = 0; i < dedupedInitData.length; i++) {
        normalisedInitData.set(dedupedInitData[i], offset);
        offset += dedupedInitData[i].length;
    }
...
shaka.polyfill.PatchedMediaKeys.v20140218.onMsKeyError_ = function(event) {
    ...
    error.errorCode = new v20140218.MediaKeyError(this.nativeMediaKeySession_.error);
